...
const Edit_product = {
        async render(id) {
            ...
            const data_category = await getAll();
            return /*html*/ `
    ...
    <div class="mt-10 sm:mt-0">
      ...
      <div class="px-4 sm:px-0">
        <h3 class="text-lg font-medium leading-6 text-gray-900">Sửa sản phẩm</h3>
        ...
        <span>4. Nhấp vào Lưu thay đổi. </span><br>
        </p>
      </div>
    </div>
              ...
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    <input type="text" value="${data.title}" id="title" autocomplete="given-name" class="p-1 mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>
    
                  <div class="col-span-6 sm:col-span-4">
                  <label for="last-name" class="block text-sm font-medium text-gray-700">Mô tả</label>
                  <textarea id="desc" name="desc" rows="3" class="p-3 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Vd: Đây là dòng nước hoa cao cấp">${data.desc}</textarea>
                  </div>
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    <select id="category" name="category" autocomplete="country-name" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                      ${data_category.data.map((itemCate) =>/* html */ `
                        <option value="${itemCate.category}" ${itemCate.category === data.category ? "selected" : ""}>${itemCate.category}</option>
                      `).join("")}
                    </select>
                  </div>
                  ...
                  <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                    <label for="city" class="block text-sm font-medium text-gray-700">Giá (đ)</label>
                    <input type="text" value="${data.price}" name="price" id="price" autocomplete="address-level2" class="p-1 mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>
    
                  <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                    <label for="region" class="block text-sm font-medium text-gray-700">Dung tích (ml)</label>
                    <input type="text" value="${data.capacity}" name="capacity" id="capacity" autocomplete="address-level1" class="p-1 mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>
                </div>
              </div>
